...
export default function Page() {
  ...
  const categories = React.useMemo(
    () => [
      ...
      ...new Set(SAMPLE_MENU.map((m) => m.category)),
    ],
    [t],
  );
  const filtered = SAMPLE_MENU.filter(
    (m) => category === t("home.categories.all") || m.category === category,
  );
        ...
        >
          {categories.map((c) => {
            const translatedCategory =
              c === t("home.categories.all") ? c : t(`categories.${c}`);
            return (
              <button
                key={c}
            ...
            >
              <div className="relative aspect-square w-full overflow-hidden bg-muted">
                <Image
                  src={
                    item.imageUrl ||
                    "/placeholder.svg?height=240&width=240&query=cafe+item"
                  }
